...
/**
 * You are given a text with a pseudographical schema of a labyrinth. Walls are
 * marked with asterisk symbols (*). Your job is to write a program that finds
 * the shortest way from the upper entrance to the lower exit and prints out the
 * labyrinth schema containing a path marked with plus symbols (+).
 * 
 * 
 * Constraints:
 * 
 * The size of a labyrinth is up to 101×101 cells. There can be more than one
 * way to pass the labyrinth, but the shortest way is always unambiguous and
 * never has branches.
 * 
 * 
 * 
...
public class Main {
	...
	public Main(char[][] labyrinth) {
		...
		marked = new boolean[labyrinth.length][labyrinth[0].length];
	...
	class Cell {
		...
		public LinkedList<Cell> adj() {
			...
			for (byte[] rowcol : adj) {
				...
				if (row + i < 0 || col + j < 0 || row + i >= labyrinth.length
						|| col + j >= labyrinth[0].length)
					continue;
				// this cell is already explored or it is a wall
				if (marked[row + i][col + j]
						|| labyrinth[row + i][col + j] == '*')
					continue;
	...
	public static void main(String args[]) throws FileNotFoundException {
		...
		Scanner textScan = new Scanner(new FileReader("labyrinth.txt"));
