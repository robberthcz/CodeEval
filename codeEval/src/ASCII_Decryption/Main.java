...
public class Main {
	...
	public Main(String sentence, char lastChar, int len) {
		...
		for (int i = 0; i < chars.length; i++) message.append(Character.toChars(chars[i] - N));
	...
	private String getWord(int i) {
		...
		for (int j = i; j < len + i; j++) toReturn += String.valueOf(chars[j]);
		return toReturn;
	}
	public static void main(String args[]) throws FileNotFoundException {
		Scanner textScan = new Scanner(new FileReader(
				"src/ASCIIDecryption/input.txt"));
