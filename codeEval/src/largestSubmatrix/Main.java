...
public class Main {
		...
		for (int a = 0; a < matrix.length; a++) {
			for (int b = a; b < matrix.length; b++) {
				...
				for (int c = 1; c < matrix[0].length; c++) {
					...
					int col;
					if (a == b)	     col = matrix[a][c];
					else if (a == 0) col = matrix[b][c];
					else			 col = matrix[b][c] - matrix[a - 1][c];
	...
	public static void main(String args[]) throws FileNotFoundException {
		Scanner textScan = new Scanner(new FileReader("src/LargestSubmatrix/input.txt"));

		String[] firstRow = textScan.nextLine().split(" ");
		int[][] matrix = new int[firstRow.length][firstRow.length];
        // parse first row
        matrix[0] = Arrays.stream(firstRow).mapToInt(Integer::parseInt).toArray();

        // parse the rest
		for(int i = 1; i < firstRow.length; i++){
            String row[] = textScan.nextLine().split(" ");
